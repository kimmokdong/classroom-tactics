    ...
    renderParticle(p, ctx, fxCanvas) {
        ...
        const lerp = (a, b, t) => a + (b - a) * t;
            ...
            if (p.type === 'low_aoe_ring') {
                ...
                ctx.fillStyle = `rgba(39, 174, 96, ${p.life / p.maxLife * 0.7})`;
                ctx.fill();
                ...
                p.vy += 1.0; p.y += p.vy;
                if(p.y >= p.targetY) { p.life = 0; this.fxSystem.spawnFx('school_pen_splash', p.x, p.targetY); }
                ctx.translate(p.x, p.y); ctx.rotate(p.rot);
                ctx.font = '28px serif'; ctx.textAlign='center'; ctx.textBaseline='middle';
                ctx.fillText('✒️', 0, 0);
            } else if (p.type === 'school_pen_mark') {
                p.progress = Math.min(p.progress + 0.015, 1);
                const t = p.life / p.maxLife;
                const s = 40 * p.progress;
                ...
                ctx.beginPath(); ctx.moveTo(p.x + s, p.y - s); ctx.lineTo(p.x - s, p.y + s); ctx.stroke();
                if(p.progress > 0.5) {
                    ctx.strokeStyle = `rgba(255, 180, 180, ${t * 0.4})`;
                    ctx.lineWidth = 1; ctx.setLineDash([3,4]);
                    ctx.beginPath(); ctx.moveTo(p.x - s * 1.2, p.y); ctx.lineTo(p.x + s * 1.2, p.y); ctx.stroke();
                    ctx.setLineDash([]);
                }
            } else if (p.type === 'school_pen_ink') {
                p.x += p.vx * 0.4; p.y += p.vy * 0.4; p.vy += 0.05;
                const t = p.life / p.maxLife;
                ctx.beginPath(); ctx.arc(p.x, p.y, p.size * t, 0, Math.PI * 2);
                ctx.fillStyle = `rgba(220, 30, 30, ${t})`; ctx.fill();
                ...
                if(p.alpha > 0) {
                    for(let i = 3; i >= 0; i--) {
                        const r = p.r * (0.5 + i * 0.15) + Math.sin(Date.now() * 0.003) * 5;
                        ctx.globalCompositeOperation = 'lighter';
                        ctx.beginPath(); ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
                ...
                ctx.font = '28px serif'; ctx.fillText('📢', p.x, p.y + 30);
                ...
                const t = p.life / p.maxLife;
                ctx.font = `${p.size}px serif`; ctx.textAlign='center'; ctx.textBaseline='middle';
                ctx.globalAlpha = t; ctx.fillText('💫', p.x, p.y);
