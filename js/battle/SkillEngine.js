export class SkillEngine {
    ...
    execute(unit, activeUnits, engine) {
        ...
        // ?¥Ïò® Ï∂©Í≤©Í∏?Í≤®Ïö∏???òÎ©¥?ëÎßê) Î∞òÏÇ¨ ?ºÌï¥
        ...
        if (unit.currHp <= 0) return; // ?§ÌÇ¨ ?∞Îã§ Ï£ΩÏúºÎ©?Ï§ëÎã®
        ...
        const applyDmg = (target, dmg, type = 'magic', isCrit = false) => {
            ...
            if (unit.combat.itemEffects?.giantSlayer) {
                finalDmg *= 1.1; // Í∏∞Î≥∏ 10% Ï¶ùÍ?
                if (target && target.stats.maxHp > 1500) finalDmg *= 1.25; // 1500 Ï¥àÍ≥º ??Ï∂îÍ? Ï¶ùÌè≠
            }
            ...
            if (type === 'physical') {
                ...
                if (unit.buffs.some(b => b.type === 'precision')) armor = 0;
                    ...
                    // Í≥µÍ≤©?êÏùò Î∞©Í? ?ÅÏö© (armorPen???àÏúºÎ©?Ï∂îÍ? Í¥Ä??
                    let armorPenMult = unit.combat.armorPen ? (1 - unit.combat.armorPen) : 1;
                    if (target.combat?.itemEffects?.gargoyle) armorPenMult = 1; // Í∞ÄÍ≥†Ïùº ?àÏúºÎ©?Î∞©Í? Î©¥Ïó≠ (?†ÌÉù ?¨Ìï≠, ?¨Í∏∞???ºÎã® ?ùÎûµ) - ?ùÎûµ
                    armor *= (1 - maxShred) * armorPenMult;
                }
                finalDmg *= (100 / (100 + armor));
                // ??dmgReduc ?ÅÏö© (?òÎìúÏ∫?75%)
                ...
                // ??dmgReduc ?ÅÏö© (?òÎìúÏ∫?75%) - ÎßàÎ≤ï ?ºÌï¥?êÎèÑ ?ÅÏö©
                let dr = target.combat?.dmgReduc || 0;
                dr += dmgReducFromCanvas;
                if (target.buffs.some(b => b.type === 'dmgReduc25')) dr += 0.25;
                dr = Math.min(dr, 0.75);
            ...
            // [?ÅÏñ¥ ?úÎÑàÏßÄ] ?§ÌÇ¨ ?ÅÏ§ë ??Î≥¥ÎÑà??ÎßàÎ≤ï ?ºÌï¥
            let engActualDmg = 0;
            ...
            // ?§ÌÇ¨ ?ºÌï¥??Í∑ºÏÑ± ÎßàÎÇò ?çÎìù ?ÅÏö© (Í≥†Ï†ï 10)
            let baseTargetGainMana = target.manaType === 'Í∑ºÏÑ±' ? 10 : 0;
            ...
            // [p15] Î∞îÎ•∏ ?ùÌôú??Î∂ÑÎÖ∏ (?âÌ? ?ºÍ≤© ??Î∞òÏÇ¨)
            if (engine.playerAugments.includes('p15') && target.team === 'player' && (Array.isArray(target.subject) ? target.subject.includes('?ÑÎçï') : target.subject === '?ÑÎçï') && unit.team === 'enemy' && finalDmg > 0) {
        ...
        switch(s.type) {
            ...
            case 'single_dot':
                if (enemies[0]) {
                    ...
                    let tickDmg = totalDmg / (s.dotDuration[starIdx] / 10); // 10??1Ï¥????∞Î?ÏßÄ
            ...
            case 'single_damage_buff':
                if (enemies[0]) {
                    ...
                    // ?§Ìîå?òÏãú Î°úÏßÅ ?òÏ†ï (hpRatioSplash??Î∞úÎèô Ï°∞Í±¥???¨Ìï®)
            ...
            case 'random_aoe_debuff':
                ...
                selected.forEach(e => {
                    ...
                    if (s.name === '?âÏùò ÎßàÎ≤ï') addBuff(e, 'manaSeal', null, 0, s.debuffDuration[starIdx]);
                });
                break;
            ...
            case 'aoe_shield_cc_immune':
                allies.forEach(a => {
                    if (engine.getDist(unit.gridIndex, a.gridIndex) <= s.aoeRange) {
                        ...
                        // ?ÑÏû¨ ?îÏßÑ Íµ¨Ï°∞??Î≥¥Ìò∏Îß??åÍ¥¥?Ä ?îÎ≤Ñ???¥Ï†úÎ•??∞Îèô?òÍ∏∞ ?¥Î†§?∞Î?Î°? 5Ï¥?50?? ÏßÄ?çÏúºÎ°?Íµ¨ÌòÑ?©Îãà??
            ...
            case 'passive':
                // ?®ÏãúÎ∏??§ÌÇ¨?Ä ?âÌ? Î£®ÌîÑ?êÏÑú Ï≤òÎ¶¨, ?¨Í∏∞?úÎäî ?ÑÎ¨¥ ?ôÏûë ?ÜÏùå
                break;
        }
