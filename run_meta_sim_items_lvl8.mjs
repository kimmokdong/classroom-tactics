...
decks.forEach(d => {
    ...
    d.units.forEach(uDef => {
        ...
        if (baseUnit) {
            ...
            const rate = SHOP_PROBS[8][tierIdx];
...
fs.writeFileSync('./scratch/meta_winrate_matrix_items_lvl8.csv', csv);
...
let report = `# 메타게임 밸런싱 분석 리포트 (8레벨 풀템 6코어 기준)\\n\\n`;
report += `## 🎲 메타 건강도 지표\\n`;
report += `- **참여 덱 수**: ${N}개\\n`;
...
fs.writeFileSync('./scratch/meta_report_items_lvl8.md', report);
console.log(`✅ 결과가 scratch/meta_report_items_lvl8.md 에 저장되었습니다!`);
