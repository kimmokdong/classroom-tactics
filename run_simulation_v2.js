...
// 데이터 임포트
import { UNIT_POOL, ITEM_POOLS } from './js/data.js';
...
// ---------------------------------------------------------------------------
function buildCombatBoard(deck, random, teamRole) {
    const board = Array(24).fill(null);
    let frontIndex = 0; // 0~7
    let backIndex = 16; // 16~23

    deck.units.forEach((unit, index) => {
        // --- 2. 성급 (Star Level) 보정 ---
        let star = 2; // 기본 2성
        
        if (deck.level === 7) {
            // 7렙 리롤덱: 캐리가 2~3코면 3성작
            if (['main_tank', 'main_dealer', 'sub_dealer'].includes(unit.assignedRole) && (unit.tier === 2 || unit.tier === 3)) {
                star = 3;
            } else if (unit.tier === 1) { // 1코 쩌리도 리롤하다 같이 3성 붙는 경우 가정
                star = 3; 
            }
        } else if (deck.level === 8) {
            // 8렙덱: 캐리가 3코면 확률적으로 3성이지만 일단 3성 고정으로 밸런스 체크, 4코는 2성
            if (['main_tank', 'main_dealer', 'sub_dealer'].includes(unit.assignedRole) && unit.tier === 3) {
                star = 3;
            }
        }

        // --- 3. 스마트 아이템 장착 (Auto-Equip) ---
        const isCore = ['main_tank', 'main_dealer', 'sub_dealer'].includes(unit.assignedRole);
        let itemIds = [];
        if (isCore) {
        ...
        const combatUnit = createUnitInstance(unit, {
            star,
            itemIds,
            instanceId: `${teamRole}:${deck.fullName}:${index}:${unit.id}`,
            teamRole,
            catalog: ITEMS,
            random
        });
...
function simulateMatch(deckA, deckB, iterations = 1000) {
    ...
    for (let i = 0; i < iterations; i++) {
        const seed = `legacy-v2:${deckA.fullName}:${deckB.fullName}:${i}`;
        const playerRaw = buildCombatBoard(deckA, createSeededRandom(`${seed}:player-build`), 'player');
        const enemyRaw = buildCombatBoard(deckB, createSeededRandom(`${seed}:enemy-build`), 'opponent');
        ...
        const engine = new BattleEngine(prepared.playerBoard, prepared.enemyBoard, [], 50, seed); // 증강 없이 순수 덱 파워만
