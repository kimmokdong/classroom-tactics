...
fs.writeFileSync('./scratch/meta_winrate_matrix_items.csv', csv);

deckStats.sort((a, b) => b.winrate - a.winrate);

let report = `# 메타게임 밸런싱 분석 리포트 (풀템 6코어 장착 기준)\\n\\n`;
report += `## 🎲 메타 건강도 지표\\n`;
report += `- **참여 덱 수**: ${N}개\\n`;
...
fs.writeFileSync('./scratch/meta_report_items.md', report);
console.log(`✅ 결과가 scratch/meta_report_items.md 에 저장되었습니다!`);
